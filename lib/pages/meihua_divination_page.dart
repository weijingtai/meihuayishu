...
class _MeiHuaDivinationPageState extends State<MeiHuaDivinationPage>
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(_inputNumber.isEmpty ? '请输入...' : _inputNumber,
                            style: TextStyle(
                                fontSize: 28,
                                fontWeight: FontWeight.bold,
                ...
                children: [
                  ...
                  TextField(
                    ...
                    onChanged: (value) async {
                      ...
                      if (value.isNotEmpty && !isLongText(value)) {
                        try {
                    ...
                    SizedBox(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          filled: true,
                          fillColor:
                              isModified ? Colors.orange.shade50 : Colors.white,
                        ),
                        onChanged: (value) {
                          final n = int.tryParse(value);
                          if (n != null && n >= 0) {
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: summary.characters.map((charAnalysis) {
                final char = charAnalysis.character;
                final pinyin = charAnalysis.pinyinWithTone;
                final strokes = charAnalysis.strokeCount;
                final originalTone = charAnalysis.modernTone;
                final currentTone = _toneOverrides[char] ?? originalTone;
                final isPing = currentTone == 1 || currentTone == 2;

                final basePinyin = charAnalysis.pinyinWithoutTone;
  ...
  Widget _buildManualTab() {
    final upperGuaName =
        _selectedUpperGua != null ? _getGuaName(_selectedUpperGua!) : null;
    final lowerGuaName =
        _selectedLowerGua != null ? _getGuaName(_selectedLowerGua!) : null;
    final isGuaComplete =
              ...
              child: Row(
                ...
                children: [
                  Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(_isXianTianGua ? '先天卦' : '后天卦',
                            style: const TextStyle(
                                fontSize: 16, fontWeight: FontWeight.bold)),
                        Text(_isXianTianGua ? '乾1兑2离3震4巽5坎6艮7坤8' : '离南坎北震东兑西',
                            style: TextStyle(
                                fontSize: 12, color: Colors.grey.shade600)),
                      ]),
                  Switch(
  ...
  Widget _buildYaoSelector(
      String? upperGuaName, String? lowerGuaName, bool isGuaComplete) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Column(
  ...
  Widget _buildRealYaoDisplay(String upperGuaName, String lowerGuaName) {
    final service = context.read<MeiHuaService>();
    final upperBinary = service.getGuaBinary(_selectedUpperGua!);
    final lowerBinary = service.getGuaBinary(_selectedLowerGua!);
    final fullBinary = lowerBinary + upperBinary;
    final yaoList = fullBinary.split('');

    return Column(
      children: [
        Text('上卦: $upperGuaName',
    ...
    return PopupMenuButton<YaoColorMode>(
      ...
      itemBuilder: (context) => YaoColorMode.values
          .map((mode) => PopupMenuItem(
              value: mode,
              child: Row(children: [
                Icon(currentMode == mode ? Icons.check : Icons.circle_outlined,
                    size: 18,
                    color:
                        currentMode == mode ? Colors.deepPurple : Colors.grey),
                const SizedBox(width: 8),
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Icon(Icons.arrow_drop_down, size: 16, color: Colors.deepPurple)
        ]),
      ),
    );
  }
      ...
      children: [
        ...
        if (themeConfig.mode == YaoColorMode.colorful)
          Text(gua.wuXing ?? '',
              style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
        const SizedBox(height: 8),
        _buildThreeYao(upperBinary, gua.upperGuaName,
            showChangingIndicator ? gua.changingYao : -1,
            startOffset: 3, showIndicator: showChangingIndicator),
        const SizedBox(height: 4),
        _buildThreeYao(lowerBinary, gua.lowerGuaName,
            showChangingIndicator ? gua.changingYao : -1,
            startOffset: 0, showIndicator: showChangingIndicator),
      ],
    );
  }

  Widget _buildThreeYao(String binary, String guaName, int changingYao,
      {required int startOffset, bool showIndicator = true}) {
  ...
  Widget _buildSingleYao(
    ...
    if (showIndicator && isChanging) {
      indicatorWidget = const Text('⭕', style: TextStyle(fontSize: 7));
    }
    return Row(
        mainAxisAlignment: MainAxisAlignment.center,
  ...
  Future<void> _generateGuaByTextWithMethod(TextDivinationMethod method) async {
    ...
    if (_toneOverrides.isNotEmpty &&
      ...
      if (method == TextDivinationMethod.bySentenceLength &&
        ...
        final analysis = calculator.getSentenceAnalysis(_inputText);
        ...
        result = await calculator
            .calculateBySentenceLengthWithOverrides(finalCounts);
      } else {
